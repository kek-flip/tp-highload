...
Google-переводчик поддерживает 133 языка на 2022 год<sup>[\[7\]](https://blog.google/products/translate/24-new-languages/)</sup>. Литературные словори содержат 200-400 тысяч слов. Для оценки положим, что в каждом языке 500 тысяч слов с учетом популярного сленга, а для каждого слова существует 3 варианта перевода. Тогда кол-во записей будет равно **(3 \* 500 тыс)^133**, а суммарный объем данных **750^133 \* 2^73 \* 10^401 эксибайт**.
...
Основываясь на оценке из предыдущего пункта, кол-во записей - **79,8 млн.**, объем данных - **4 ГБайт**.
...
| WordTranslation | Хранит возможные переводы слов                                                                     | **(3 \* 500 тыс)^133** записей, **750^133 \* 2^73 \* 10^401 эксибайт** данных |       -       |  955,5 тыс.   |
|    WordCount    | Хранит кол-во слов в текстах. Используется для 5-грамм                                             | **79,8 млн.** записей, **4 ГБайт** данных                                     |       -       |  955,5 тыс.   |
