...
Nginx является обратным прокси-сервером, обеспечивающим доступ к сервису и выполняющим ssl-терминацию и локальную балансировку нагрузки, которая работает по алгоритму round-robin. Трафик же к экземплярам nginx доставляет система глобальной балансировки, организованной на основе BGP-anycast.

Входной точкой в сервис является микросервис front. Данный микросервис занимается сбором статистики и данных пользовательских запросов для дальнейшего обучения моделей. И исходный, и переведенный тексты, а также изображения, front асинхронно отправляет в kafka и на S3.

После этого пользовательский запрос попадает в front переводов, который агрегирует результаты работы трех сервисов - парсера страниц, сервиса переводов и сервиса распознавания картинок - чтобы затем отдать собранный воедино ответ обратно в сервис front, затем в nginx и далее до конечного пользователя. Здесь важно отметить наличие S3-буфера у сервиса распознавания изображений. Он был добавлен для отказоустойчивости, так как сервису переводов небходимо хранить промежуточные изображения для распознавания.

Данные из kafka данные будет доставать микросервис парсинга данных, который будет предобрабатывать данные и синхронизировать данные с S3 и класть эти данные в ClickHouse и neo4j.

Кроме того, есть микросервис по обучению и замене моделей. Он берет данные из СУБД и с их помощью обучает модели и обновляет их на сервисах переводов и распознавания изображений.
