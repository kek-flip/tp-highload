...
| Text     | Текст. Хранит как исходные, так и переведенные. В эту же таблицу добавлятся тексты с сайтов     |
...
Данная структура данных представляет собой 5-мерный массив, элемент fiveGramm[a][b][c][d][e] равен вероятности того, что слова **a, b, c, d и e** идут подряд в преложении. То есть, данная структура данных позволяет подобрать наиболее вероятное слово на осонове предыдущего контекста.  
...
Одна запись = **50 Байт**  
Ежедневный объем данных - **1,915 млрд** строк, **89 ГБайт** данных.
...
Одна запись = **64 Байт**  
Ежедневный объем данных - **1,915 млрд** строк, **114 ГБайт** данных.
...
Одная запись <= **1033 Байт**. Среднее значение, исходя из расчетов в [пункте 2](#2-расчет-нагрузки) ~ **581 Байт**.
Ежедневный объем данных - **305 млн.** записей, **48 ГБайт** данных, **80 ГБайт** - в худшем случае.
