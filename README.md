...
- ### [Список серверов](#11-список-серверов)
...
Тогда **сетевой трафик = позитивный трафик + негативный трафик = (610 млн. \* 0,375 \* 1 байт + 610 млн. \* 0,125 \* (1 байт + 137 \* 2 байт)) 86400 с = 0,002 Гбит/c**,  
...
Было принято решение не использовать L3 балансировщики, так как они не дают сильного преимущества при заметно большей сложности, чем L7 балансировщики.

Ключевые моменты работы nginx:

- Балансировка - балансировка будет осуществляться по алгоритму round-robin без весов
- Ssl-терминация - трафик внутри сети сервисов будет передаваться по HTTP, что позволит выиграть в скорости
- Обратный прокси-сервер - Nginx будет использоваться как обратный прокси-сервер для сокрытия доступа к сервисам извне
- Сжатие данных - Nginx будет сжимать изображения для переводов с помощью
- Повторные запросы - в случае неудачного запроса Nginx как frontend-сервер попытается повторить запрос
- Логгирование - все запросы на переводы, а также их результаты, будут логгироваться для дальнейшей аналитики по ним
...
Расчеты ниже будут предполагать, что мы сохраняем все запросы, ответы и изображения.
...
На основе каждого запроса будем дополнять 5-граммы, поэтому появится нагрузка на чтение, равная суммарному RPS по текстам, изображениям и веб-страницам.

#### Хранение изображений

Как было подсчитано в [пункте 2](#2-расчет-нагрузки), для изображений **суммарный суточный трафик = 129600 ГБайт**. Чтобы уменьшить занимаемое изображениями место на диске, будем хранить изображения в течение двух недель, по истечению которых они будут удаляться. Кроме того, будем хранить не только исходное, но и переведенное изображение.

Итого, общий объем памяти для изображений будет равен **14 \* 129600 ГБайт = 3628800 ГБайт ≈ 3544 ТБайт**.
...
|    Text     | Текст. Хранит как исходные, так и переведенные. В эту же таблицу будут добавляться тексты с сайтов | **\*155 млрд.** записей, **354 ТБайт** данных, в худшем случае - **708 ТБайт** |  105,75 тыс.  |       -       |
|  5-граммы   | Вероятности того, что 5 слов идут подряд в предложении                                             | **8,4305 \* 10^9 эксабайт**                                                    |       -       |  212,5 тыс.   |
| Изображения | Изображения. Хранятся как исходные, так и переведенные                                             | **3544 ТБайт**                                                                 |     2016      |       -       |

## 6 Физическая схема базы данных

### Денормализация и шардирование
...
Шардирование будет выполнено по принципу одна таблица - один шард.
...
- Для сбора результатов перевода и обратной связи имеется отдельный микросервис stat2k, который складывает исходный и переведенный тексты, ссылки на изображения и отправляет эти данные в kafka.
...
Согласно данным бенчмарка <sup>[\[10\]](https://www.techempower.com/benchmarks/#section=data-r22&hw=ph&test=fortune)</sup> удельные значения ресурсов, которые будут потреблять сервисы, равны **0,2 ядра** и **0,7 ГБ ОЗУ** на **1000 RPS** для сервисов на `Go/Gin` и **10 ядер** и **1 ГБ ОЗУ** на **1000 RPS** для моделей на `PyTorch`. Зная это, вычислим кол-во ресурсов для каждого сервиса.

| Сервис                           | RPS<sub>пиковый</sub> | CPU, ядра | RAM, ГБайт |
| -------------------------------- | --------------------- | --------- | ---------- |
| Перевод веб-страниц              | 176,25 тыс.           | 35,25     | 123,375    |
| Перевод текстов                  | 352,5 тыс.            | 70,5      | 246,75     |
| Перевод изображений              | 2520                  | 0,5       | 1,76       |
| Модель перевода текстов          | 531,27 тыс.           | 5312,7    | 531,27     |
| Модель распознавания изображений | 5040                  | 50,4      | 5,04       |
| stat2k                           | 796,27 тыс.           | 159,25    | 557,39     |
| Агрегатор                        | 796,27 тыс.           | 159,25    | 557,39     |

### Нагрузка на хранилища

Пусть, как и во втором пункте, мы находимся в "добром" интернете, и 90% запросов к модели переводов находят нужные слова в кэше, а в сам кэш пишутся слова из половины запросов.

Нагрузки на чтение для всех систем, кроме Redis, не являются постоянными, так как данные из них читаются при необходимости обучения моделей.

Для расчета аппаратных ресурсов воспользуемся следующими пропорциями:

- На **1000 RPS** необходимо **2 ядра** и **1,25 ГБайт ОЗУ**
- Данные о запросах будут храниться в течение двух недель, а запросы, которые будут связаны с обратной связью - в течение полугода. То есть, дневные метрики по объему из [пункта 6](#6-физическая-схема-базы-данных) нужно преобразовать по формуле **0,5 \* М \* 14 + 0,5 \* М \* 180 = 97 \* М**.

| Хранилище            | RPS чтение  | RPS запись  | CPU, ядра | RAM, ГБайт | Диск                         |
| -------------------- | ----------- | ----------- | --------- | ---------- | ---------------------------- |
| neo4j                | 212,5 тыс.  | -           | 425       | 265,625    | 8,4305 \* 10^9 эксабайт      |
| ClickHouse. Feedback | -           | 159,25 тыс. | 318,5     | 199        | 7760 ГБайт                   |
| ClickHouse. Request  | -           | 318,5 тыс.  | 637       | 398,125    | _722136_ \* 97 = 68405 ТБайт |
| ClickHouse. Respose  | -           | 318,5 тыс.  | 637       | 398,125    | _722157_ \* 97 = 68407 ТБайт |
| Redis                | 478,14 тыс. | 265,64 тыс. | 956,3     | 597,675    | -                            |
| S3                   | -           | 2016        | -         | -          | 3544 ТБайт                   |

P.S Курсивом выделены персчитанные метрики согласно денормализованным таблицам

### Конфигурация серверов

| Название                         | Хостинг | Конфигурация                                         | Cores | Count            | Стоимость (1 шт.) |
| -------------------------------- | ------- | ---------------------------------------------------- | ----- | ---------------- | ----------------- |
| Перевод веб-страниц              | own     | Intel Xeon Platinum 8356H/2x16GB DDR4/1xNVMe 256GB   | 8     | 5                | $3480             |
| Перевод текстов                  | own     | Intel Xeon Platinum 8356H/2x16GB DDR4/1xNVMe 256GB   | 8     | 10               | $3480             |
| Перевод изображений              | own     | Intel Xeon Platinum 8356H/1x8GB DDR4/1xNVMe 256GB    | 8     | 1                | $3440             |
| Модель перевода текстов          | own     | 2xAMD EPYC 9654/2x16GB DDR5/1xNVMe 256GB             | 192   | 28               | $23725            |
| Модель распознавания изображений | own     | Intel Xeon Platinum 8356H/2x16GB DDR4/1xNVMe 256GB   | 8     | 7                | $3480             |
| stat2k                           | own     | 2xIntel Xeon Platinum 8356H/4x16GB DDR4/1xNVMe 256GB | 16    | 10               | $6880             |
| Агрегатор                        | own     | 2xIntel Xeon Platinum 8356H/4x16GB DDR4/1xNVMe 256GB | 16    | 10               | $6880             |
| neo4j                            | own     | AMD EPYC 9254/2x16GB DDR5/8xHDD 28TB                 | 24    | 3,94645 \* 10^13 | $7990             |
| ClickHouse. Feedback             | own     | AMD EPYC 9254/2x8GB DDR5/1xHDD 1TB                   | 24    | 14               | $2400             |
| ClickHouse. Request              | own     | AMD EPYC 9254/2x16GB DDR5/8xHDD 28TB                 | 24    | 305              | $7990             |
| ClickHouse. Respose              | own     | AMD EPYC 9254/2x16GB DDR5/8xHDD 28TB                 | 24    | 305              | $7990             |
| Redis                            | own     | AMD EPYC 9254/2x16GB DDR5/1xNVMe 256GB               | 24    | 19               | $2410             |
| S3                               | own     | Intel Xeon Platinum 8356H/2x8GB DDR4/1xHDD 20TB      | 8     | 178              | $3480             |

### Распределение ресурсов в оркестрации

| Сервис                           | CPU/r | CPU/l | RAM/r | RAM/l | Count |
| -------------------------------- | ----- | ----- | ----- | ----- | ----- |
| Перевод веб-страниц              | 8     | 8     | 16GB  | 32GB  | 5     |
| Перевод текстов                  | 8     | 8     | 16GB  | 32GB  | 10    |
| Перевод изображений              | 8     | 8     | 16GB  | 32GB  | 1     |
| Модель перевода текстов          | 8     | 16    | 16GB  | 32GB  | 664   |
| Модель распознавания изображений | 8     | 16    | 16GB  | 32GB  | 7     |
| stat2k                           | 8     | 8     | 16GB  | 32GB  | 20    |
