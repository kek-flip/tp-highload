...
На входе дата-центра будут располагаться L3 Virtual Server via IP Tunneling балансировщики, которые будет первично распределять нагрузку.

Следующем находятся слой машин, которые будут осуществлять ssl-терминацию. Вынес ssl-терминации на отдельные машины вызван спецификой сервиса: для онлайн-переводчика характерно создание большого количества соединений с малым временем жизни, следовательно, ssl будет давать ощутимую нагрузку на сервис, поэтому стоит вынести их отдельно. Расшифрованный трафик будет отдаваться L7 балансировщикам.

В качестве L7 балансировщика будет использоваться Nginx, который будет балансировать нагрузку уже между конечными серверами сервиса.
